...
Refer to the definitive list of rules below, but here are some quick key points

- DO NOT use the `any` type, except in certain circumstances described below.
- Prefer inferring types vs strictly typing where possible. i.e in `.map(el)` or return types.
- Prefer `satisfies` over `as` for type assertions.
- DO NOT invent API request/response types (our code-gen generates these)
- Co-locate types where they are used
...
- Use camelCase for file names (e.g., `myComponent.ts`)
