...
import type { SystemCtx, AsyncSystemFn, SyncSystemFn } from "@/core/SystemCtx";
...
describe("Engine", () => {
  ...
  describe("system registration", () => {
    ...
    it("should support method chaining for system registration", () => {
      ...
      const exitSystem: AsyncSystemFn = vi.fn();
  ...
  describe("engine lifecycle", () => {
    it("should run initialization stages when started", async () => {
    ...
    it("should start the animation frame loop", async () => {
      await engine.run();
    ...
    it("should provide correct system context", async () => {
      const testSystem: AsyncSystemFn = vi.fn();
    ...
    it("should allow systems to stop the engine", async () => {
      const stopperSystem: AsyncSystemFn = (ctx: SystemCtx) => {
        ctx.stop();
      };
  ...
  describe("game loop", () => {
    it("should update time in the loop", async () => {
    ...
    it("should execute update stages in correct order", async () => {
      const executionOrder: string[] = [];
      ...
      engine
        ...
        .addSystem(Schedule.Tick, () => {
          executionOrder.push("tick");
        })
        .addSystem(Schedule.Update, () => {
    ...
    it("should handle fixed timestep updates", async () => {
    ...
    it("should update event bus at frame boundary", async () => {
    ...
    it("should schedule next frame when running", async () => {
      let loopCallback: Function | null = null;
      mockRequestAnimationFrame.mockImplementation((callback: Function) => {
        loopCallback = callback;
        return 123;
  ...
  describe("stopping the engine", () => {
    ...
    it("should provide system context without stop function in exit stage", async () => {
      const exitSystem: AsyncSystemFn = vi.fn();
      engine.addSystem(Schedule.Exit, exitSystem);
  ...
  describe("integration scenarios", () => {
    it("should handle complete engine lifecycle", async () => {
      const lifecycle: string[] = [];

      engine
        .addSystem(Schedule.PreInit, () => {
          lifecycle.push("preInit");
        })
        .addSystem(Schedule.Init, () => {
          lifecycle.push("init");
        })
        .addSystem(Schedule.PostInit, () => {
          lifecycle.push("postInit");
        })
        .addSystem(Schedule.PreUpdate, () => {
          lifecycle.push("preUpdate");
        })
        .addSystem(Schedule.Update, () => {
          lifecycle.push("update");
        })
        .addSystem(Schedule.PostUpdate, () => {
          lifecycle.push("postUpdate");
        })
        .addSystem(Schedule.Exit, () => {
          lifecycle.push("exit");
        });
    ...
    it("should handle system that throws during initialization", async () => {
      const errorSystem: AsyncSystemFn = () => {
    ...
    it("should handle system that throws during update loop", async () => {
    ...
    it("should handle complex fixed timestep scenarios", async () => {
      let tickCount = 0;
      const tickSystem: SyncSystemFn = () => {
        tickCount++;
      };
  ...
  describe("performance considerations", () => {
    it("should handle many systems efficiently", async () => {
    ...
    it("should not accumulate memory leaks across frames", async () => {
      let loopCallback: Function | null = null;
      mockRequestAnimationFrame.mockImplementation((callback: Function) => {
        loopCallback = callback;
        return 123;
  ...
  describe("edge cases", () => {
    it("should handle requestAnimationFrame returning null", async () => {
    ...
    it("should handle missing animation frame APIs gracefully", async () => {
      ...
      await expect(async () => {
        await engine.run();
      }).rejects.toThrow(); // Should throw since RAF is undefined
